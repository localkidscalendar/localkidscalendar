-- Admin "Override 3+": reinstate content and exempt from future community auto-hide.
-- Clear Flags remains a second chance (resets count, clears exemption).
-- Users may still flag exempt items; Admin still sees reports and can manually deactivate.

alter table public.events
  add column if not exists flag_auto_hide_exempt boolean not null default false;
...
-- Allow overridden on flag case history (3+ cards)
do $$
declare
  t text;
begin
  foreach t in array array['events', 'comments', 'ad_library', 'banner_ads']
  loop
    ...
    begin
      execute format(
        'alter table public.%I add constraint %I check (
          ...
          or flag_case_admin_action in (''manually_deactivated'', ''reactivated'', ''overridden'', ''reviewed'', ''flags_cleared'')
        )',
        t,
...
drop function if exists public.admin_notify_owner_flag_lifecycle(uuid, text, uuid, text, numeric, text, text, text);
drop function if exists public.admin_notify_owner_flag_lifecycle(uuid, text, uuid, text, numeric, text, text, text, boolean);
...
begin
  ...
  return public.notify_owner_flag_lifecycle(
    ...
    p_flag_count, p_reason, p_details, p_item_label, p_auto_hidden
  );
end;
$$;

grant execute on function public.admin_notify_owner_flag_lifecycle(uuid, text, uuid, text, numeric, text, text, text, boolean) to authenticated;
grant execute on function public.admin_notify_owner_flag_lifecycle(uuid, text, uuid, text, numeric, text, text, text, boolean) to service_role;
