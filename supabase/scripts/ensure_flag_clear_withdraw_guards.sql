-- Flag clear / withdraw guards + suspended cannot flag content
-- 1) submit_flag: block suspended/disabled via is_account_action_blocked()
-- 2) withdraw_*: refuse Admin-cleared reports (re-flag stays blocked)
...
-- Block withdraw of Admin-cleared reports (keeps re-flag blocked)
...
begin
  ...
  select fr.id, fr.admin_action
  into v_report_id, v_admin_action
  ...
  if coalesce(v_admin_action, '') = 'flag_cleared' then
    raise exception 'This flag was cleared by an Admin and cannot be removed';
  end if;
...
begin
  ...
  if p_target_type = 'ad' then
    ...
    select fr.id, fr.admin_action into v_report_id, v_admin_action
    from public.flag_reports fr
