-- Mass messages: no default action button; optional Admin-provided link
...
declare
  ...
  v_action_label text := nullif(trim(coalesce(p_action_label, '')), '');
  v_action_href text := nullif(trim(coalesce(p_action_href, '')), '');
  r record;
begin
  ...
  -- Action only when both label and href are provided
  if v_action_label is null or v_action_href is null then
    v_action_label := null;
    v_action_href := null;
  end if;
...
-- Drop older overloads so PostgREST resolves the new signature cleanly
drop function if exists public.send_mass_message(text, text, text, text[]);
drop function if exists public.send_mass_message(text, text, text[], text[]);

grant execute on function public.send_mass_message(text, text, text[], text[], text, text) to authenticated;
grant execute on function public.send_mass_message(text, text, text[], text[], text, text) to service_role;
