...
export default function AdminDefaultAdsPanel({ toast }) {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const items = await base44.entities.AdminDefaultAd.list("-priority", 50);
      setAds(items);
    } catch { setAds([]); }
    setLoading(false);
  };
  ...
  const handleCreate = async () => {
    ...
    setSaving(true);
    try {
      await base44.entities.AdminDefaultAd.create({
  ...
  const handleToggleSlot = async (ad, slot) => {
    // Unassign this slot from any other ad first
    const currentHolder = ads.find(a => a[slot] && a.id !== ad.id);
    if (currentHolder) {
      await base44.entities.AdminDefaultAd.update(currentHolder.id, { [slot]: false });
    }
    const newVal = !ad[slot];
    await base44.entities.AdminDefaultAd.update(ad.id, { [slot]: newVal });
  ...
  const handleToggleStatus = async (ad) => {
    ...
    await base44.entities.AdminDefaultAd.update(ad.id, { status: newStatus });
  ...
  const handleDelete = async (ad) => {
    ...
    await base44.entities.AdminDefaultAd.delete(ad.id);
    setAds(prev => prev.filter(a => a.id !== ad.id));
    toast({ title: "Default ad deleted" });
  };

  // Which ad is assigned to each slot
