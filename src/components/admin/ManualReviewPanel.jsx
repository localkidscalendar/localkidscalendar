...
export default function ManualReviewPanel({ toast }) {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const fetched = await base44.entities.AdLibrary.filter({ moderation_status: "manual_review" }, "-created_date", 100);
      setRequests(fetched);
      const userIds = [...new Set(fetched.map(r => r.user_id).filter(Boolean))];
      if (userIds.length > 0) {
        const allUsers = await base44.entities.User.list();
        const map = {};
        allUsers.forEach(u => { map[u.id] = u.full_name || u.email || u.id; });
        setUsers(map);
      }
    } catch { setRequests([]); }
    setLoading(false);
  ...
  const handleApprove = async (item) => {
    ...
    try {
      await base44.entities.AdLibrary.update(item.id, {
  ...
  const sorted = [...requests].sort((a, b) => {
    let aVal = sortBy === "user_name" ? (users[a.user_id] || "").toLowerCase() : (a.created_date || "");
    let bVal = sortBy === "user_name" ? (users[b.user_id] || "").toLowerCase() : (b.created_date || "");
          ...
          <table className="w-full text-sm">
            <thead className="bg-purple-50">
              <tr>
                ...
                <th className="text-left px-4 py-2.5 font-medium text-purple-700 cursor-pointer hover:bg-purple-100 select-none" onClick={() => toggleSort("created_date")}>
                  <span className="flex items-center gap-1">Requested {arrow("created_date")}</span>
                </th>
