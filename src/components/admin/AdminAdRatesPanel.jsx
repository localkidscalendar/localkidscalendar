...
export default function AdminAdRatesPanel({ toast }) {
  ...
  const [form, setForm] = useState({ monthly_rate: "", annual_discount_percent: "", effective_date: "", notes: "" });
  ...
  const load = async () => {
    ...
    try {
      const [configs, hist] = await Promise.all([
        ...
        base44.entities.AdPricingHistory.list("-effective_date", 50),
      ]);
      if (configs.length > 0) setCurrent(configs[0]);
      setHistory(hist);
    } catch {}
    setLoading(false);
  };
  ...
  const handleSave = async () => {
    ...
    if (isNaN(monthly) || isNaN(discount) || monthly <= 0 || discount < 0 || discount >= 100) {
    ...
    setSaving(true);
    try {
      // Close out the previous current rate's end_date if it exists
      if (current) {
        // Find the history record for the current rate (most recent without end_date)
        const openRecord = history.find(h => !h.end_date);
        if (openRecord) {
          const dayBefore = moment(form.effective_date).subtract(1, "day").format("YYYY-MM-DD");
          await base44.entities.AdPricingHistory.update(openRecord.id, { end_date: dayBefore });
        }
        // Update the global config
        await base44.entities.AdPricingConfig.update(current.id, {
          monthly_rate: monthly,
          annual_discount_percent: discount,
        });
      } else {
        // Create global config
        await base44.entities.AdPricingConfig.create({
          config_key: "global",
          monthly_rate: monthly,
          annual_discount_percent: discount,
        });
      ...
      // Create a new history record (no end_date = currently active)
      await base44.entities.AdPricingHistory.create({
        monthly_rate: monthly,
        annual_discount_percent: discount,
      ...
      toast({ title: "New rate saved and will take effect on " + moment(form.effective_date).format("MMM D, YYYY") });
  ...
  if (loading) return <div className="flex justify-center py-8"><Loader2 className="w-5 h-5 animate-spin text-mint-500" /></div>;

  const annualPrice = current ? Math.round(current.monthly_rate * 12 * (1 - current.annual_discount_percent / 100)) : null;
    ...
    <div className="bg-white border border-border rounded-2xl p-5 space-y-5">
      ...
      <div>
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <Button size="sm" className="rounded-xl bg-mint-500 hover:bg-mint-600 text-white" onClick={() => setShowForm(!showForm)}>
        ...
        {current ? (
          <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-3">
            ...
            <div className="bg-muted/30 rounded-xl p-3 text-center">
              ...
              <p className="text-xs text-muted-foreground">Annual Plan ({current.annual_discount_percent}% off)</p>
            </div>
            <div className="bg-muted/30 rounded-xl p-3 text-center">
        ...
        <div className="border-t border-border pt-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Input
                type="number" min="0" max="99" placeholder="30"
          ...
          <div>
            ...
            />
            <p className="text-xs text-muted-foreground mt-1">Rate goes live on this date. Prior rate is closed out the day before.</p>
          </div>
          <div>
          ...
          {form.monthly_rate && form.annual_discount_percent && (
            <div className="bg-muted/40 rounded-xl p-3 text-sm">
              ...
              <p>Monthly: <strong>${parseFloat(form.monthly_rate || 0).toFixed(0)}/mo</strong></p>
              <p>Annual: <strong>${Math.round(parseFloat(form.monthly_rate || 0) * 12 * (1 - parseFloat(form.annual_discount_percent || 0) / 100)).toLocaleString()}/yr</strong></p>
            </div>
          )}
          <div className="flex gap-2">
            <Button variant="outline" className="rounded-xl" onClick={() => setShowForm(false)}>Cancel</Button>
            <Button className="rounded-xl bg-mint-500 hover:bg-mint-600 text-white" disabled={saving} onClick={handleSave}>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-border">
                {history.map(h => {
                  const annual = Math.round(h.monthly_rate * 12 * (1 - h.annual_discount_percent / 100));
                  const isActive = !h.end_date;
                  return (
                    <tr key={h.id} className={isActive ? "bg-muted/30" : ""}>
                      <td className="px-3 py-2">
                        {moment(h.effective_date).format("MMM D, YYYY")}
                        {isActive && <span className="ml-2 text-xs text-mint-500 font-semibold">● Current</span>}
                      </td>
                      <td className="px-3 py-2 text-muted-foreground">{h.end_date ? moment(h.end_date).format("MMM D, YYYY") : "—"}</td>
